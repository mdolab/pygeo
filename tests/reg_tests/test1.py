...
class RegTestPyGeo(unittest.TestCase):
    ...
    def test_1(self, train=False, refDeriv=False):
        ...
        """
        refFile = os.path.join(self.base_path,'ref/test1.ref')
    ...
    def test_2(self, train=False, refDeriv=False):
        ...
        """
        refFile = os.path.join(self.base_path,'ref/test2.ref')
        with BaseRegTest(refFile, train=train) as handler:
    ...
    def test_3(self, train=False, refDeriv=False):
        ...
        """
        refFile = os.path.join(self.base_path,'ref/test3.ref')
        with BaseRegTest(refFile, train=train) as handler:
    ...
    def test_4(self, train=False, refDeriv=False):
        ...
        """
        refFile = os.path.join(self.base_path,'ref/test4.ref')
        with BaseRegTest(refFile, train=train) as handler:
    ...
    def test_5(self, train=False, refDeriv=False):
        ...
        """
        refFile = os.path.join(self.base_path,'ref/test5.ref')
        with BaseRegTest(refFile, train=train) as handler:
    ...
    def train_6a(self, train=True, refDeriv=True):
        self.test_6a(train=train, refDeriv=refDeriv)

    def test_6a(self, train=False, refDeriv=False):
        """
        Test 6a: Basic + Nested FFD, local DVs only
        """
        refFile = os.path.join(self.base_path,'ref/test6a.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 6a: Basic + Nested FFD, local DVs only")
            DVGeo,DVGeoChild = commonUtils.setupDVGeo(self.base_path)
    ...
    def train_6b(self, train=True, refDeriv=True):
        self.test_6b(train=train, refDeriv=refDeriv)

    def test_6b(self, train=False, refDeriv=False):
        """
        Test 6b: Basic + Nested FFD, local DVs only on parent
        """
        refFile = os.path.join(self.base_path,'ref/test6b.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 6b: Basic + Nested FFD, local DVs only on parent")
            DVGeo,DVGeoChild = commonUtils.setupDVGeo(self.base_path)
    ...
    def train_6c(self, train=True, refDeriv=True):
        self.test_6c(train=train, refDeriv=refDeriv)

    def test_6c(self, train=False, refDeriv=False):
        """
        Test 6c: Basic + Nested FFD, local DVs only on child
        """
        refFile = os.path.join(self.base_path,'ref/test6c.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 6c: Basic + Nested FFD, local DVs only on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeo(self.base_path)
    ...
    def test_7(self, train=False, refDeriv=False):
        """
        Test 7: Basic + Nested FFD, local DVs only on parent, global on child
        """
        refFile = os.path.join(self.base_path,'ref/test7.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 7: Basic + Nested FFD, local DVs only on parent, global on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeo(self.base_path)
    ...
    def test_8(self, train=False, refDeriv=False):
        """
        Test 8: Basic + Nested FFD, local DVs only on parent, global and local on child
        """
        refFile = os.path.join(self.base_path,'ref/test8.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 8: Basic + Nested FFD, local DVs only on parent, global and local on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeo(self.base_path)
    ...
    def test_9(self, train=False, refDeriv=False):
        """
        Test 9: Basic + Nested FFD, global DVs and local DVs on parent local on child
        """
        refFile = os.path.join(self.base_path,'ref/test9.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 9: Basic + Nested FFD, global DVs and local DVs on parent local on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeo(self.base_path)
...
# -------------------
    ...
    def test_10(self, train=False, refDeriv=False):
        """
        Test 10: D8 FFD, global DVs
        """
        refFile = os.path.join(self.base_path,'ref/test10.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 10: D8 FFD, global DVs")
            ...
            handler.root_print("Test 10b: D8 FFD, random DV perturbation on test 10")
            xDV = DVGeo.getValues()
            for key in xDV:
    ...
    def test_11(self, train=False, refDeriv=False):
        """
        Test 11: D8 FFD, global DVs and local DVs
        """
        refFile = os.path.join(self.base_path,'ref/test11.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 11: D8 FFD, global DVs and local DVs")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
            ...
            handler.root_print("Test 11b: D8 FFD, random DV perturbation on test 11")
            xDV = DVGeo.getValues()
    ...
    def test_12(self, train=False, refDeriv=False):
        ...
        Test 12
        """
        refFile = os.path.join(self.base_path,'ref/test12.ref')
        with BaseRegTest(refFile, train=train) as handler:            
            handler.root_print("Test 12: D8 + Nozzle FFD, global DVs only")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
            ...
            handler.root_print("Test 12b: D8 + Nozzle FFD, random DV perturbation on test 12")
            xDV = DVGeo.getValues()
            for key in xDV:
                numpy.random.seed(42)
                xDV[key]+=numpy.random.rand(len(xDV[key]))
    ...
    def test_13(self, train=False, refDeriv=False):
        ...
        Test 13
        """
        refFile = os.path.join(self.base_path,'ref/test13.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 13: D8 + Nozzle FFD, global DVs and local DVs on parent global on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
            ...
            handler.root_print("Test 13b: D8 + Nozzle FFD, random DV perturbation on test 13")
            xDV = DVGeo.getValues()
            for key in xDV:
                numpy.random.seed(42)
                xDV[key]+=numpy.random.rand(len(xDV[key]))
    ...
    def test_14(self, train=False, refDeriv=False):
        ...
        Test 14
        """
        refFile = os.path.join(self.base_path,'ref/test14.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 14: D8 + Nozzle FFD,  global DVs and local DVs on both parent and child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
    ...
    def test_15(self, train=False, refDeriv=False):
        ...
        Test 15
        """
        refFile = os.path.join(self.base_path,'ref/test15.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 15: D8 + Nozzle FFD, local DVs only")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
            ...
            handler.root_print("Test 15b: D8 + Nozzle FFD, random DV perturbationon test 15")
            xDV = DVGeo.getValues()
            for key in xDV:
                numpy.random.seed(42)
                xDV[key]+=numpy.random.rand(len(xDV[key]))
    ...
    def test_16(self, train=False, refDeriv=False):
        ...
        Test 16
        """
        refFile = os.path.join(self.base_path,'ref/test16.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 16: D8 + Nozzle FFD, local DVs only on parent, global on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
            ...
            handler.root_print("Test 16b: D8 + Nozzle FFD, random DV perturbationon test 16")
            xDV = DVGeo.getValues()
            for key in xDV:
                numpy.random.seed(42)
                xDV[key]+=numpy.random.rand(len(xDV[key]))
    ...
    def test_17(self, train=False, refDeriv=False):
        ...
        Test 17
        """
        refFile = os.path.join(self.base_path,'ref/test17.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 17: D8 + Nozzle FFD, local DVs only on parent, global and local on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
            ...
            handler.root_print("Test 17b: D8 + Nozzle FFD,  random DV perturbationon test 17")
            xDV = DVGeo.getValues()
            for key in xDV:
                numpy.random.seed(42)
                xDV[key]+=numpy.random.rand(len(xDV[key]))
    ...
    def test_18(self, train=False, refDeriv=False):
        ...
        Test 18
        """
        refFile = os.path.join(self.base_path,'ref/test18.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 18: D8 + Nozzle FFD, global DVs and local DVs on parent local on child")
            DVGeo,DVGeoChild = commonUtils.setupDVGeoD8(self.base_path, refDeriv)
            ...
            handler.root_print("Test 18b: D8 + Nozzle FFD,  random DV perturbationon test 18")
            xDV = DVGeo.getValues()
            for key in xDV:
                numpy.random.seed(42)
                xDV[key]+=numpy.random.rand(len(xDV[key]))
    ...
    def test_19(self, train=False, refDeriv=False):
        ...
        Test 19
        """
        refFile = os.path.join(self.base_path,'ref/test19.ref')
        with BaseRegTest(refFile, train=train) as handler:
            handler.root_print("Test 19: Axisymmetric FFD, global and local DVs")
    ...
    def test_20(self, train=False, refDeriv=False):
        ...
        Test 20
        """
        refFile = os.path.join(self.base_path,'ref/test20.ref')
        with BaseRegTest(refFile, train=train) as handler:
