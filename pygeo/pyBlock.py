...
class pyBlock:
    ...
    def _readPlot3D(self, fileName, order="f", FFD=False, symmTol=0.001, ku_max=4, kv_max=4, kw_max=4):
        ...
        if FFD:
            ...
            for ivol in range(nVol):
                ku = min(ku_max, sizes[ivol, 0])
                kv = min(kv_max, sizes[ivol, 1])
                kw = min(kw_max, sizes[ivol, 2])
