...
class DVGeometryVSPOld(object):
    ...
    def addPointSet(self, points, ptName, **kwargs):
        ...
        # INSTEAD, call VSP to project these points onto the geometry.
        # then just keep the default calculations wrt offset stuff etc.

        if len(points) > 0:

            # Only need to change here really
...
# ----------------------------------------------------------------------
    ...
    def _getUpdatedSurface(self, fName='export.x'):
        ...
        # INSTEAD, we just need the new coordinates of our initial projected points.


        # Write the export file.
        fName = os.path.join(self.tmpDir, fName)

        # vsp exports file here
    ...
    # we wont even need this with the in-memory approach
    ...
    def _computeSurfJacobian(self):
        ...
        for iDV in range(len(dvKeys)):
            ...
            if iDV % self.comm.size == self.comm.rank:
                ...
                # revert back to the original design. We are doing this again here
                # because there is some hysteresis in openVSP.

                # DV = self.DVs[dvKeys[iDV]]
                # # We use float here since sometimes pyoptsparse will give
                # # stupid numpy zero-dimensional arrays, which swig does
                # # not like.
                # vsp.SetParmVal(DV.parmID, float(DV.value))
...
class CompIntersection(object):
    def __init__(self, compA, compB, extraComps, direction, dStar, pts, cumSizes, sizes, tmpDir):
        ...
        # s is the number of points on this surface, cumSizes have the cumilative sizes, easier to access the points from the big array
